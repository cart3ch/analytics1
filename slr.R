#Simple Linear Regression (only one independent variable)- Case Study
# Regression : Areas vs Sales
#Given data of area and sales, predict value for sales (dependent variable) for specific areas (independent area) eg : (1.5,2,3,4,5)
#manual way of doing regression see online URL
#Linear Regressions has a lot of assumptions like
#the residuals should be nomally distributed
#con only interpolate the values no extrapolation
...
fit1
summary(fit1) # if p-value of F statistic is less than 0.05 then things are significant and there's a linear model existing, atleast one independent variable having a relation with the dependent variable
#R_sq => % of variation in y explained by variation in x, >0.6 to be considered a good fit
#sd around the prediction line => residual std err
...
plot(fit1, which=1) #plot(fit1) gives four plots, which can be used to check assumptions

par(mfrow=c(2,2)) #when residual do not follow a funnel pattern then a linear fit exists Homocedacity
plot(fit1)
par(mfrow=c(1,1))
...
#auto collinearity today's sales data depends on yestersday's sales data
