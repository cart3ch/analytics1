...
colnames(data)
class(data$Age)
apply(data, 2, FUN= class)  #checks class of each column are all numeric
dim(data)
head(data)
summary(data)
...
k1<-amap::Kmeans(data[,-1],centers=3, iter.max = 200,nstart = 1, 
                 method = c("euclidean")) # the -1 is for removing cust_id , clustering will not give results with rollno, cust_id etc
