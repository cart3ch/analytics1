...
#My own attempt at using the piping operator---- 
#always write in paper today what output you want and then you use 
names(sales)
sales%>%filter(region== '9x-Export')%>%group_by(custname)%>%summarise(mr=max(revenue))%>%arrange(desc(mr))
